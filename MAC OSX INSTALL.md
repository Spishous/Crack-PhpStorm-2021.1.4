# Crack-PhpStorm-2021.1.4 for MAC OSX
## Step 1:
- Installer Phpstorm 2021.1.4<br/>
link: [https://www.jetbrains.com/fr-fr/phpstorm/download/other.html](https://www.jetbrains.com/fr-fr/phpstorm/download/other.html)
- Activate the trial version (do not fill in the information and validate)


## Step 2:
- Copy Paste the file "BetterIntelliJ-1.20.jar" to your user directory<br/>
(Drag and drop in "/Users/lastprophet/" )

## Step 3:
- Open phpstorm
- go to help → edit custom VM options
- Add this line to the file:
```
-javaagent:/Users/lastprophet/BetterIntelliJ-1.20.jar
```
- If previous VM Option didn't work, Go to Finder > Applications Right click on PHPStorm and clic "Show content package" Open folder Contents > bin and edit "phpstorm.vmoptions" and add this line at the end: 
```
-javaagent:/Users/lastprophet/BetterIntelliJ-1.20.jar
```
Save file!
`Attention: check that it is the same path as the file put in step 2`

## Step 4:
```
$ sudo nano /etc/hosts
```
- Add these 3 lines:
```
127.0.0.1 account.jetbrains.com
127.0.0.1 jrebel.npegeek.com
127.0.0.1 oauth.account.jetbrains.com
127.0.0.1 bs.studycoder.com
```
Save and exit!

## Step 5:
- Enter the activation code and you're done

>BISACXYELK---

<img src='https://user-images.githubusercontent.com/53474519/200335186-516fcea5-5a2b-44f1-a47b-33dc5ec0838e.png' width='600'>
